  ...
  try {
    ...
    if (userFound) {
      ...
      if (pswdmatch) {
        ...
        req.app.locals.specialContext = "mobile number and password are invalid"
        res.redirect("/login");
      }
    } else {
...
exports.singleProduct = async (req,res,next)=>{
  try {
    ...
    if(user){
       ...
       wishlistFound = await Wishlist.findOne({user},{product:{$elemMatch:{product_id:id}}})
    }
    ...
    if(singleProduct){
      res.render('singleproduct',{singleProduct,productFound,wishlistFound})
    }
...
exports.editProfile = async (req,res)=>{
  try {
    ...
    if(data){
      await User.findByIdAndUpdate({_id:id},{$set:{name,mobile,email}})
